...
- SQLAlchemy 2.x для ORM-моделей и репозиторного слоя;
- Alembic для миграций.

### База данных

- PostgreSQL;
- локально PostgreSQL поднимается в Docker через официальный образ `postgres`;
...
- `POST /profiles` - создание persisted профиля пользователя;
- `GET /profiles` - список сохраненных профилей для повторного использования;
- `GET /profiles/{profile_id}` - получение профиля;
- `PATCH /profiles/{profile_id}` - изменение профиля.

### Planning

- `POST /trip-requests` - создание persisted запроса на планирование поездки со snapshot профиля;
- `POST /recommendations/generate` - генерация персонализированных рекомендаций, реализовано на seed-данных;
- `POST /routes/build` - построение persisted маршрута с расчетом расстояния, времени, бюджета и возврата в отель;
- `GET /routes/{route_id}` - получение построенного persisted маршрута.
...
Базовая формула ранжирования остается факторной:
...
В текущей MVP-реализации поверх базовой формулы добавлены поправки профиля "цифрового сознания 2.0":

- `must_have` повышают score подходящих POI;
- `avoid` дают штраф для конфликтующих POI;
- `compromise_strategy` меняет веса базовых факторов;
- `trust_level` регулирует степень допуска goal-driven и exploratory рекомендаций;
- `goals` расширяют допустимые категории и влияют на final ranking;
- `accessibility_needs` и `preferred_time_windows` дают дополнительные бонусы или штрафы, которые отражаются и в explanation.
...
В текущей backend-версии seed-данные уже автоматически обогащаются при загрузке:

- interests дополняются и нормализуются автоматически;
- вычисляется `interest_source`;
- строятся `source_links`;
- для новых импортов могут автоматически вычисляться `data_quality_score` и `popularity_score`;
- модель POI готова к приему изображений, Wikipedia/Wikidata и media provenance.
...
Для перехода от ручного seed-набора к масштабируемому источнику данных добавлен ETL-скелет:

- `backend/app/services/data_import.py` — нормализация OpenTripMap, Overpass и Wikimedia;
- `backend/app/services/poi_enrichment.py` — автоматическое enrichment существующих POI;
- `backend/app/tools/seed_enrichment_report.py` — audit покрытия текущего seed-набора;
- `docs/data-ingestion-plan.md` — план хранения POI, изображений и внешних source links.
